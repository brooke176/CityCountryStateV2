...
class BattleModeManager {
    ...
    func handleIncomingMessage(components: URLComponents) {
        ...
        // Example: Update current letter from incoming message
        if let letterValue = components.queryItems?.first(where: { $0.name == "letter" })?.value {
            currentLetter = letterValue
            updateLetterDisplay()
        }

        // Example: Update player scores if present
        for (index, _) in players.enumerated() {
            let key = "player\(index + 1)Score"
            if let scoreString = components.queryItems?.first(where: { $0.name == key })?.value,
               let score = Int(scoreString) {
                players[index].score = score
            }
        }

        // Refresh UI after applying incoming message data
        GameManager.shared.updatePlayerUI()
        updateUI()
    }
